...
{
//to preform the fft you need to execute the fft code before the decimation of the samples
// execute this on the dma buffer not the fft_signal
//	arm_rfft_fast_init_f32(&fft_handler, 1024);
//	if(currentSettings.fft == 1)
//	{
//		for (int i = 0; i < 1024; i++)
//		{
			//for flattop do on original samples
			...
			//for hanning do on original samples
//			fft_in_buf[i] = fft_signal[i] * (0.5 - 0.5 * cos(2 * PI * i / (1024)));

			//for uniform don nothing with the smaples
//		}
//		arm_rfft_fast_f32(&fft_handler, fft_in_buf, fft_out_buf, 0);
//		arm_cmplx_mag_f32(fft_in_buf, fft_out_buf, 1024);
//		for getting the frequencies on the indexes you need to use the formula sample_rate / FFT_size, this is done on the interface
//	}
	//HAL_SPI_TransmitReceive(&hspi1, fft_out_buf, spi_rx_buf, SPI_TX_BUF_SIZE, 1000);
